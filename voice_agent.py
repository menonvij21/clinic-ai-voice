...
def think(user_text: str, memory: dict) -> str:
    ...
    if sess["step"] == "get_slot":
        ...
        # Slot not available — offer next 2 alternatives
        alternatives = [s for s in avail if s != chosen][:2]
        if alternatives:
            alt_str = " or ".join([fmt_slot(s) for s in alternatives])
            return (
                f"I'm sorry, {fmt_slot(chosen)} is not available. "
    ...
    if sess["step"] == "confirm":
        ...
        if any(x in tl for x in ["yes","confirm","correct","sure","ok","go ahead","please"]):
            ...
            memory["session"] = get_session()  # reset for next call
